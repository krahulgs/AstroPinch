...
class PDFReportService:
    ...
    @staticmethod
    def _add_standard_branding(canvas, doc):
        PDFReportService._draw_branding(canvas, doc, "Premium Astrology Report")

    @staticmethod
    def _add_compatibility_branding(canvas, doc):
        PDFReportService._draw_branding(canvas, doc, "Premium Compatibility Report")

    @staticmethod
    def _draw_branding(canvas, doc, title):
        canvas.saveState()
        ...
        canvas.drawRightString(523, 800, title)
    ...
    def generate_pdf_report(report_data, lang="en"):
        ...
        vedic_data = report_data.get('vedic_astrology') or {}
        raw_planets = vedic_data.get('planets') or []
        ...
        if raw_planets:
            ...
            try:
                ...
                img_ni = Image(chart_ni_buf, width=3.0*inch, height=3.0*inch)
                ...
                asc_info = vedic_data.get('ascendant') or {}
                ...
                img_si = Image(chart_si_buf, width=3.0*inch, height=3.0*inch)
                
                # Layout in a table
                visual_table = Table([[img_ni, img_si]], colWidths=[3.1*inch, 3.1*inch])
                ...
                cap_table = Table(captions, colWidths=[3.1*inch, 3.1*inch])
                cap_table.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'CENTER')]))
                story.append(cap_table)

                # --- Navamsa Chart (D9) Section ---
                story.append(Spacer(1, 0.4*inch))
                story.append(Paragraph("Divisional Chart (Navamsa / D9)", styles['SectionHeader']))
                
                # 1. North Indian Navamsa (Diamond)
                nav_ni_buf = ChartGenerator.generate_navamsa_chart_north_indian(vedic_data.get('divisional_charts') or {})
                img_nav_ni = Image(nav_ni_buf, width=3.0*inch, height=3.0*inch)
                
                # 2. South Indian Navamsa (Square)
                # Map D9 planets for SI
                d9_data = (vedic_data.get('divisional_charts') or {}).get('D9') or []
                si_planets_d9 = []
                for p in d9_data:
                    si_planets_d9.append({
                        "planet": p.get('name'),
                        "sign_id": sign_map.get(p.get('sign'), 1),
                        "degree": "00" # D9 signs are enough
                    })
                
                # Navamsa Ascendant for SI
                asc_lon = (vedic_data.get('ascendant') or {}).get('longitude') or 0
                try:
                ...
                nav_asc_sign_idx = (start_offsets[element_group] + (pada - 1)) % 12
                
                si_planets_d9.append({
                    "planet": "Asc",
                    "sign_id": nav_asc_sign_idx + 1,
                    "degree": "00"
                })

                nav_si_buf = ChartGenerator.generate_south_indian_chart_image(si_planets_d9)
                img_nav_si = Image(nav_si_buf, width=3.0*inch, height=3.0*inch)
                
                # Layout
                nav_table = Table([[img_nav_ni, img_nav_si]], colWidths=[3.1*inch, 3.1*inch])
                nav_table.setStyle(TableStyle([
                    ('ALIGN', (0,0), (-1,-1), 'CENTER'),
                    ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
                ]))
                story.append(nav_table)
                # Define a fresh caption table for Navamsa
                nav_cap_table = Table(captions, colWidths=[3.1*inch, 3.1*inch])
                nav_cap_table.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'CENTER')]))
                story.append(nav_cap_table)
        ...
        if panchang:
            ...
            panchang_data = [
                ["Tithi", html.escape(str((panchang.get('tithi') or {}).get('name', 'N/A')))],
                ["Nakshatra", html.escape(f"{(panchang.get('nakshatra') or {}).get('name', 'N/A')} (Lord: {(panchang.get('nakshatra') or {}).get('lord', 'N/A')})")],
                ["Yoga", html.escape(str((panchang.get('yoga') or {}).get('name', 'N/A')))],
                ["Karana", html.escape(str((panchang.get('karana') or {}).get('name', 'N/A')))],
                ["Ascendant", html.escape(str((panchang.get('ascendant') or {}).get('name', 'N/A')))]
            ]
        ...
        # Loshu Grid
        loshu = report_data.get('numerology', {}).get('loshu_grid')
        if loshu:
            ...
            if loshu.get('completed_planes'):
                story.append(Paragraph(f"Completed Planes: {', '.join(map(str, loshu.get('completed_planes', [])))}", styles['NormalText']))
            
            # Add Missing Number Remedies
            remedies_loshu = loshu.get('remedies')
            if remedies_loshu and isinstance(remedies_loshu, dict):
                story.append(Spacer(1, 0.1*inch))
                story.append(Paragraph("<b>Key Remedies (Missing Numbers):</b>", styles['NormalText']))
                for num, rem in remedies_loshu.items():
                    story.append(Paragraph(f"• #{num}: {rem}", styles['NormalText']))
            
            story.append(Spacer(1, 0.2*inch))

        # --- Science of Success (Hilary Gerard) ---
        success_data = (report_data.get('numerology') or {}).get('science_of_success')
        if success_data and isinstance(success_data, dict):
            story.append(Paragraph("Science of Success (Hilary Gerard System)", styles['SectionHeader']))
            story.append(Paragraph(f"<b>Fadic Number: {success_data.get('fadic_number')}</b> - {success_data.get('fadic_type')}", styles['NormalText']))
            story.append(Paragraph(f"<i>Symbol: {success_data.get('symbol')}</i>", styles['NormalText']))
            story.append(Paragraph(success_data.get('description', ''), styles['NormalText']))
            
            story.append(Paragraph("<b>Positive Qualities:</b>", styles['NormalText']))
            qualities = success_data.get('qualities') or {}
            story.append(Paragraph(html.escape(str(qualities.get('positive') or 'N/A') if isinstance(qualities, dict) else str(qualities)), styles['NormalText']))
            
            story.append(Paragraph("<b>Challenges to Overcome:</b>", styles['NormalText']))
            story.append(Paragraph(html.escape(str(qualities.get('negative') or 'N/A') if isinstance(qualities, dict) else ''), styles['NormalText']))
            
            # Guidelines
            guidance = success_data.get('guidance') or {}
            guide_data = [
                ["Lucky Days", ", ".join(map(str, guidance.get('lucky_days', [])))],
                ["Lucky Colors", ", ".join(map(str, guidance.get('lucky_colors', [])))],
                ["Lucky Gems", ", ".join(map(str, guidance.get('lucky_gems', [])))],
                ["Success Years", f"Next destiny year: {success_data.get('next_destiny_year', 'N/A')}"]
            ]
            gt = Table(guide_data, colWidths=[1.5*inch, 4.5*inch])
        ...
        if acg_data:
            ...
            for loc in acg_data:
                acg_table_data.append([
                    ...
                    html.escape(str(loc.get('line_title') or '')),
                    html.escape(str(loc.get('effect_title') or ''))
                ])
            ...
            story.append(Spacer(1, 0.2*inch))
            story.append(Paragraph("Power Zone Highlights", styles['SectionHeader']))
            for loc in acg_data[:5]:
                 if isinstance(loc, dict):
                    story.append(Paragraph(f"<b>{html.escape(str(loc.get('city') or ''))}</b>: {html.escape(str(loc.get('description') or ''))}", styles['NormalText']))
        ...
        if remedies:
            ...
            if mantra:
                ...
                story.append(Paragraph(f"<b>Deity:</b> {html.escape(str(mantra.get('deity') or 'N/A'))}", styles['NormalText']))
                story.append(Paragraph(f"<b>Mantra:</b> {html.escape(str(mantra.get('sanskrit') or 'N/A'))}", styles['NormalText']))
                story.append(Paragraph(f"<i>Instructions: {html.escape(str(mantra.get('instructions') or 'N/A'))}</i>", styles['NormalText']))
        ...
        if predictions and isinstance(predictions, dict):
            ...
            best_p = predictions.get('best_prediction')
            if best_p:
                story.append(Paragraph(html.escape(str(best_p)), styles['NormalText']))

        doc.build(story, onFirstPage=PDFReportService._add_standard_branding, onLaterPages=PDFReportService._add_standard_branding)
        buffer.seek(0)
        return buffer
    ...
    def generate_kundali_match_pdf(result, lang="en"):
        ...
        if 'transit_analysis' in result:
            ...
            if isinstance(transits, dict) and transits.get('analysis'):
                story.append(Spacer(1, 0.1*inch))
        ...
        doc.build(story, onFirstPage=PDFReportService._add_compatibility_branding, onLaterPages=PDFReportService._add_compatibility_branding)
        buffer.seek(0)
        return buffer
