...
class PDFReportService:
    ...
    def generate_kundali_match_pdf(result, lang="en"):
        ...
        TEXT = {
            "en": {
                "title": "Vedic Compatibility & Gun Milan Report",
                "ashta_koota": "Ashta-Koota Breakdown",
                "manglik": "Manglik Analysis",
                "dosha": "Dosha Analysis",
                "advanced": "Advanced Compatibility Analysis",
                "dasha_sync": "Dasha Synchronization Analysis",
                "navamsa": "Navamsa (D9) Compatibility",
                "transits": "Current Planetary Transits",
                "marriage_windows": "Auspicious Marriage Timing Windows",
                "ai_guidance": "AI Cosmic Guidance & Expert Insights",
                "footer_premium": "Premium Compatibility Report",
                "footer_generated": "Generated by AstroPinch AI Expert",
                "page": "Page",
                "bride": "Bride",
                "groom": "Groom",
                "koota": "Koota",
                ...
                "bride_val": "Bride Val",
                "groom_val": "Groom Val",
                "manglik_bride": "Bride Status",
                "manglik_groom": "Groom Status",
                "overall_score": "Overall Guna Score",
                "result": "Result",
                "marital_harmony": "Marital Harmony Score",
                "transit_favorability": "Transit Favorability",
                "overall_assessment": "Overall Assessment",
                "recommendation": "Recommendation",
                "current_dasha": "Current Dasha"
            },
            "hi": {
                "title": "वैदिक अनुकूलता और गुण मिलान रिपोर्ट",
                "ashta_koota": "अष्टकूट मिलान विवरण",
                "manglik": "मांगलिक दोष विश्लेषण",
                "dosha": "दोष विश्लेषण",
                "advanced": "विस्तृत अनुकूलता विश्लेषण",
                "dasha_sync": "दशा अनुकूलता विश्लेषण",
                "navamsa": "नवांश (D9) अनुकूलता",
                "transits": "ग्रह गोचर फल",
                "marriage_windows": "विवाह के लिए शुभ मुहूर्त",
                "ai_guidance": "AI ज्योतिषीय मार्गदर्शन और एक्सपर्ट राय",
                "footer_premium": "प्रीमियम अनुकूलता रिपोर्ट",
                "footer_generated": "एस्ट्रोपंच AI विशेषज्ञ द्वारा निर्मित",
                "page": "पृष्ठ",
                "bride": "वधू",
                "groom": "वर",
                "koota": "कूट",
                ...
                "bride_val": "वधू पक्ष",
                "groom_val": "वर पक्ष",
                "manglik_bride": "वधू की स्थिति",
                "manglik_groom": "वर की स्थिति",
                "overall_score": "कुल गुण मिलान",
                "result": "परिणाम",
                "marital_harmony": "वैवाहिक सामंजस्य स्कोर",
                "transit_favorability": "गोचर अनुकूलता",
        ...
        t_labels = TEXT.get(lang, TEXT["en"])
        ...
        # Title
        story.append(Spacer(1, 0.5*inch))
        story.append(Paragraph(t_labels['title'], styles['CenterTitle']))
        story.append(Spacer(1, 0.2*inch))

        # Names
        # Names
        name_style = ParagraphStyle(name='NameStyle', parent=styles['Normal'], fontSize=12, alignment=1, spaceAfter=5, fontName=bold_font)
        story.append(Paragraph(f"<b>{result['bride']['name']}</b> ({t_labels['bride']}) & <b>{result['groom']['name']}</b> ({t_labels['groom']})", name_style))
        story.append(Spacer(1, 0.3*inch))
        ...
        store_result_text = f"{t_labels['result']}: {result['summary']}"
        story.append(Paragraph(f"{t_labels['overall_score']}: {score}/36", styles['ScoreHighlight']))
        story.append(Paragraph(store_result_text, styles['NormalText']))
        story.append(Spacer(1, 0.3*inch))

        # Ashta-Koota Breakdown
        story.append(Paragraph(t_labels['ashta_koota'], styles['SectionHeader']))
        table_data = [[t_labels['koota'], t_labels['significance'], t_labels['bride_val'], t_labels['groom_val'], t_labels['points']]]
        ...
        # Manglik Analysis
        story.append(Paragraph(t_labels['manglik'], styles['SectionHeader']))
        story.append(Paragraph(f"{t_labels['manglik_bride']}: <b>{result['bride']['manglik_status']}</b>", styles['NormalText']))
        story.append(Paragraph(f"{t_labels['manglik_groom']}: <b>{result['groom']['manglik_status']}</b>", styles['NormalText']))
        story.append(Paragraph(result['manglik_summary'], styles['NormalText']))
        story.append(Spacer(1, 0.3*inch))

        # Dosha Analysis
        # Dosha Analysis
        story.append(Paragraph(t_labels['dosha'], styles['SectionHeader']))
        ...
        # Advanced Compatibility Analysis
        story.append(Paragraph(t_labels['advanced'], styles['CenterTitle']))
        story.append(Spacer(1, 0.2*inch))

        # Dasha Synchronization
        if 'dasha_synchronization' in result:
            dasha_sync = result['dasha_synchronization']
            story.append(Paragraph(t_labels['dasha_sync'], styles['SectionHeader']))
            ...
            story.append(Paragraph(f"<b>{t_labels['recommendation']}:</b> {dasha_sync.get('recommendation', 'N/A')}", styles['NormalText']))
        ...
        if 'navamsa_compatibility' in result:
            ...
            story.append(Paragraph(t_labels['navamsa'], styles['SectionHeader']))
            story.append(Paragraph(f"<b>{t_labels['marital_harmony']}:</b> {navamsa.get('score', 0)}/10", styles['NormalText']))
            story.append(Paragraph(f"<b>{t_labels['recommendation']}:</b> {navamsa.get('recommendation', 'N/A')}", styles['NormalText']))
        ...
        if 'transit_analysis' in result:
            ...
            story.append(Paragraph(t_labels['transits'], styles['SectionHeader']))
            story.append(Paragraph(f"<b>{t_labels['transit_favorability']}:</b> {transits.get('score', 0)}/10", styles['NormalText']))
            story.append(Paragraph(f"<b>{t_labels['overall_assessment']}:</b> {transits.get('recommendation', 'N/A')}", styles['NormalText']))
        ...
        if 'marriage_windows' in result:
            ...
            story.append(Paragraph(t_labels['marriage_windows'], styles['SectionHeader']))
            story.append(Paragraph(f"<b>{t_labels['recommendation']}:</b> {windows.get('recommendation', 'N/A')}", styles['NormalText']))
        ...
        # AI Insights
        story.append(Paragraph(t_labels['ai_guidance'], styles['CenterTitle']))
        ai_text = result['ai_analysis'].replace('\n', '<br/>')
        story.append(Paragraph(ai_text, styles['NormalText']))

        def add_branding(canvas, doc):
            canvas.saveState()
            # Branding uses Hindi font if lang is Hi, but Helvetica-Bold is standard for English name
            # Ideally "AstroPinch" remains English or gets transliterated. Keeping "AstroPinch" as brand
            # But the footer text changes
            ...
            # Right Header
            if lang == "hi":
                canvas.setFont(HINDI_FONT, 8)
            else:
                canvas.setFont('Helvetica', 8)
            
            canvas.setFillColor(colors.gray)
            canvas.drawRightString(523, 800, t_labels['footer_premium'])
            
            # Left Footer
            canvas.drawString(72, 30, t_labels['footer_generated'])
            
            # Right Footer
            canvas.drawRightString(523, 30, f"{t_labels['page']} {doc.page}")
            canvas.restoreState()
